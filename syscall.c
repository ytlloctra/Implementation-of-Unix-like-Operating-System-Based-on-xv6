...
extern int sys_mqget(void);   //messagequeue.c
extern int sys_msgsnd(void);
extern int sys_msgrcv(void);
extern int sys_myalloc(void);
extern int sys_myfree(void);

// extern int clone(void (*)(void *), void *, void *);
// extern int join(void **);

extern int sys_clone(void);
extern int sys_join(void);

static int (*syscalls[])(void) = {   //以eax及系统调用号 在 syscalls[]系统调用表中找到需要执行的代码  系统调用跳转表
[SYS_fork]    sys_fork,
[SYS_exit]    sys_exit,
[SYS_wait]    sys_wait,
[SYS_pipe]    sys_pipe,
...
{
  int num;
  // 获取在usys.S中设置到eax中的系统调用号   取出trapframe里保存的eax系统调用号
  num = proc->tf->eax;
  // 根据系统调用号到syscalls数组中取对应的系统调用函数进行调用
  if(num > 0 && num < NELEM(syscalls) && syscalls[num]) {   // 调用号在合法范围内
    proc->tf->eax = syscalls[num]();  
  } else {
    cprintf("%d %s: unknown sys call %d\n",
            proc->pid, proc->name, num);
    proc->tf->eax = -1;   // 非法的系统调用号  返回-1
  }
}
