...
void            wakeup(void*);
void            yield(void);
int             getcpuid(void);   //为了让sysproc.c中的sys_getcpuid()可以调用proc.c中的getcpuid()
int             chpri(int, int);
int             join(void **);
...
// messagequeue.c
void            mqinit();  //消息队列初始化
int             mqget(uint); //申请使用消息队列
int             msgsnd(uint, void*, int); //向消息队列发出消息
int             msgrcv(uint, void*, int); // 从消息队列接收消息
void            releasemq2(int);
void			releasemq(uint);
